...
CLI="${PROJECT_DIR}/bin/linux-canva-web"
PASSED=0
...
APP_ID="mnnncmnfcndlghkomlbaadoonfnjcadg"
mkdir -p \
  "${TEST_HOME}" \
...
export LINUX_CANVA_WEB_LAUNCHER="${CLI}"
...
CONFIG="${XDG_CONFIG_HOME}/linux-canva-web/config"
DESKTOP="${XDG_DATA_HOME}/applications/linux-canva-web.desktop"
ICON="${XDG_DATA_HOME}/icons/hicolor/256x256/apps/linux-canva-web.png"
assert_file "${CONFIG}"
assert_file "${DESKTOP}"
assert_file "${ICON}"
...
assert_contains "${TEMP_ROOT}/wrong-app.err" "none matches the current official Canva ID"
...
assert_contains "${TEMP_ROOT}/wrong-explicit.err" "does not match the current official Canva manifest ID"
...
DEB_ROOT_MODE="$(dpkg-deb --fsys-tarfile "${PROJECT_DIR}/dist/linux-canva-web_${VERSION}_all.deb" | tar -tvf - | awk 'NR == 1 {print $1}')"
