...
[[ -f "${PROJECT_DIR}/bin/linux-canva-web" ]] || { echo "Project files are incomplete." >&2; exit 1; }

install -d "${PREFIX}/bin" "${PREFIX}/share/doc/linux-canva-web"
install -m 0755 "${PROJECT_DIR}/bin/linux-canva-web" "${PREFIX}/bin/linux-canva-web"
install -m 0644 \
  "${PROJECT_DIR}/README.md" \
  "${PROJECT_DIR}/LICENSE" \
  "${PROJECT_DIR}/SECURITY.md" \
  "${PROJECT_DIR}/THIRD_PARTY_NOTICES.md" \
  "${PREFIX}/share/doc/linux-canva-web/"

printf 'Installed: %s\n' "${PREFIX}/bin/linux-canva-web"
case ":${PATH}:" in
  *":${PREFIX}/bin:"*) ;;
...
if [[ "${RUN_SETUP}" == true ]]; then
  LINUX_CANVA_WEB_LAUNCHER="${PREFIX}/bin/linux-canva-web" \
    "${PREFIX}/bin/linux-canva-web" setup
else
  printf 'Next: %s setup\n' "${PREFIX}/bin/linux-canva-web"
fi
