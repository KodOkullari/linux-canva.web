...
VERSION="$("${PROJECT_DIR}/bin/linux-canva-web" version | awk '{print $2}')"
...
install -d \
  "${BUILD_DIR}/DEBIAN" \
  "${BUILD_DIR}/usr/bin" \
  "${BUILD_DIR}/usr/share/doc/linux-canva-web"
chmod 0755 "${BUILD_DIR}"
install -m 0644 "${PROJECT_DIR}/packaging/deb/DEBIAN/control" "${BUILD_DIR}/DEBIAN/control"
install -m 0755 "${PROJECT_DIR}/bin/linux-canva-web" "${BUILD_DIR}/usr/bin/linux-canva-web"
install -m 0644 \
...
mkdir -p "${DIST_DIR}"
PACKAGE="${DIST_DIR}/linux-canva-web_${VERSION}_all.deb"
